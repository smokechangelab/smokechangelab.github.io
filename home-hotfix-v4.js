...
function inject(){
 if(q('#homeHotfixV5'))return;
 const s=document.createElement('style');s.id='homeHotfixV5';s.textContent=`
 ...
 .primary-action-v4{min-width:0!important;min-height:54px;border-radius:15px;border:1px solid #E2E8F0;padding:0 12px;font-size:11px;font-weight:850;line-height:1.2;display:flex;align-items:center;justify-content:center;text-align:center;box-shadow:0 10px 24px rgba(27,79,76,.06);white-space:normal;word-break:normal;overflow-wrap:normal;hyphens:none}
 .primary-action-v4.smoke{background:#E8873A;border-color:#E8873A;color:#fff}
 .primary-action-v4.resist{background:#fff;border-color:#1B4F4C;color:#1B4F4C}
 ...
 #onboarding.active{align-items:center!important;padding:0!important}
 #onboarding .onboard{max-width:448px!important;width:100%!important;margin:0 auto!important;gap:12px!important}
 #onboarding .setup-card{padding:20px!important;border-radius:18px!important}
 #onboarding .onboard-top{margin:0!important;padding:0 0 14px!important;display:flex!important;align-items:center!important;gap:10px!important;border-bottom:1px solid #EDF1EF!important}
 #onboarding .onboard-top.wizard-integrated{margin-bottom:14px!important}
 #onboarding .step-box{min-width:0!important;flex:1 1 auto!important}
 #onboarding .step-row{font-size:9.5px!important;line-height:1.2!important}
 #onboarding .track{margin-top:6px!important;width:100%!important}
 #onboarding .choice-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}
 #onboarding .choice{min-width:0!important;overflow-wrap:normal!important;word-break:normal!important;hyphens:none!important}
 .bottom-nav .tab{font-size:11px!important}.bottom-nav .tab .tab-icon{font-size:20px!important}
 @media(max-width:390px){.tool-card{height:142px!important;min-height:142px!important;max-height:142px!important;padding:14px!important}.primary-action-v4{font-size:10.3px!important}.bottom-nav .tab{font-size:9.5px!important}.bottom-nav .tab .tab-icon{font-size:18px!important}#floatingSosV4{right:16px;width:50px;height:50px;border-radius:17px}#onboarding .setup-card{padding:18px!important}}
 `;document.head.appendChild(s);
}
function buildPrimaryActions(){
 const shell=q('.dash-v3');if(!shell)return;
 const old=q('#heroSmoke',shell);if(old)old.remove();
 let wrap=q('#homePrimaryActionsV4');
 if(!wrap){wrap=document.createElement('div');wrap.id='homePrimaryActionsV4';wrap.innerHTML='<button id="heroSmokeV4" class="primary-action-v4 smoke" type="button">+ Zigarette erfassen</button><button id="heroResistV4" class="primary-action-v4 resist" type="button">Rauchdrang widerstanden</button>';shell.appendChild(wrap)}
 const wanted=['statusCardV3','homePrimaryActionsV4','dayRadarV3','labInsightV3'];
 wanted.forEach(id=>{const el=q('#'+id,shell);if(el)shell.appendChild(el)});
}
function buildFab(){
 ...
 const nav=q('#bottomNav');const sync=()=>b.classList.toggle('show',!!nav?.classList.contains('show'));
 sync();if(nav)new MutationObserver(sync).observe(nav,{attributes:true,attributeFilter:['class']});
}
function integrateOnboarding(){
 const onboard=q('#onboarding .onboard'),top=q('#onboarding .onboard-top'),card=q('#onboarding #obContent .setup-card');
 if(!onboard||!top||!card)return;
 if(top.parentElement!==card){card.prepend(top)}
 top.classList.add('wizard-integrated');
}
function observeOnboarding(){
 const ob=q('#onboarding');if(!ob)return;
 const mo=new MutationObserver(()=>requestAnimationFrame(integrateOnboarding));
 mo.observe(ob,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
 integrateOnboarding();
}
function wire(){
...
function apply(){inject();buildPrimaryActions();buildFab();integrateOnboarding()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{apply();observeOnboarding()},{once:true});else{apply();observeOnboarding()}

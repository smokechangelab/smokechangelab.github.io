(()=>{
  ...
  const close=()=>{craving?.classList.remove('open','sos-picker-open');intervention?.classList.remove('open')};
  const current=()=>state.key?DATA[state.key]:null;

  function picker(){
    close();state={key:null,round:1};craving.classList.add('open','sos-picker-open');
    ...
    cravingSub.textContent=t('Ein Tap genügt.','One tap is enough.');
    cravingBody.innerHTML=`<button class="sos-x" data-micro-close aria-label="${t('Schließen','Close')}">×</button><div class="micro-trigger-grid">${Object.entries(DATA).map(([k,x])=>`<button class="micro-trigger" data-micro-trigger="${k}"><span class="micro-trigger-icon" aria-hidden="true">${x.icon}</span><b>${t(x.label[0],x.label[1])}</b></button>`).join('')}</div>`;
  ...
  function impulse(key,round=1){
    ...
    craving.classList.remove('open','sos-picker-open');intervention.classList.add('open');
